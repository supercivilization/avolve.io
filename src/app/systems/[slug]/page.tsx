...
export async function generateStaticParams() {
  const articles = getAllArticles('systems')
  return articles.map(article => ({ slug: article.slug }))
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params
  const article = await getArticleBySlug('systems', slug)
  ...
  return {
    ...
    openGraph: {
      ...
      url: `https://avolve.io/systems/${slug}`,
    },
  }
}
      ...
      <article className="prose prose-lg dark:prose-invert max-w-4xl mx-auto px-4 py-12">
        ...
        <header className="mb-8 not-prose">
          <div className="flex items-center gap-2 mb-4 text-sm">
            <span className="px-3 py-1 bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-full font-medium">
              Systems
            </span>
