...
export async function generateStaticParams() {
  const articles = getAllArticles('support')
  return articles.map(article => ({ slug: article.slug }))
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params
  const article = await getArticleBySlug('support', slug)
  ...
  return {
    ...
    openGraph: {
      ...
      url: `https://avolve.io/support/${slug}`,
    },
  }
}
...
export default async function SupportPage({ params }: Props) {
  ...
  const url = `https://avolve.io/support/${slug}`
  const articleSchema = generateArticleSchema(frontmatter, url)
  
  const breadcrumbItems = [
    { name: 'Home', url: '/' },
    { name: 'Support', url: '/support' },
    { name: frontmatter.title, url: `/support/${slug}` },
  ]
      ...
      <article className="prose prose-lg dark:prose-invert max-w-4xl mx-auto px-4 py-12">
        ...
        <header className="mb-8 not-prose">
          <div className="flex items-center gap-2 mb-4 text-sm">
            <span className="px-3 py-1 bg-stone-100 dark:bg-stone-800 text-stone-900 dark:text-stone-100 rounded-full font-medium">
              Support
            </span>
