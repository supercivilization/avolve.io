...
export async function generateStaticParams() {
  const articles = getAllArticles('solutions')
  return articles.map(article => ({ slug: article.slug }))
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params
  const article = await getArticleBySlug('solutions', slug)
  ...
  return {
    ...
    openGraph: {
      ...
      url: `https://avolve.io/solutions/${slug}`,
    },
  }
}
...
export default async function SolutionsPage({ params }: Props) {
  ...
  const url = `https://avolve.io/solutions/${slug}`
  const articleSchema = generateArticleSchema(frontmatter, url)
  
  const breadcrumbItems = [
    { name: 'Home', url: '/' },
    { name: 'Solutions', url: '/solutions' },
    { name: frontmatter.title, url: `/solutions/${slug}` },
  ]
      ...
      <article className="prose prose-lg dark:prose-invert max-w-4xl mx-auto px-4 py-12">
        ...
        <header className="mb-8 not-prose">
          <div className="flex items-center gap-2 mb-4 text-sm">
            <span className="px-3 py-1 bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-slate-100 rounded-full font-medium">
              Solutions
            </span>
