...
export async function generateStaticParams() {
  const articles = getAllArticles('software')
  return articles.map(article => ({ slug: article.slug }))
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params
  const article = await getArticleBySlug('software', slug)
  ...
  return {
    ...
    openGraph: {
      ...
      url: `https://avolve.io/software/${slug}`,
    },
  }
}
...
export default async function SoftwarePage({ params }: Props) {
  ...
  const url = `https://avolve.io/software/${slug}`
  const articleSchema = generateArticleSchema(frontmatter, url)
  
  const breadcrumbItems = [
    { name: 'Home', url: '/' },
    { name: 'Software', url: '/software' },
    { name: frontmatter.title, url: `/software/${slug}` },
  ]
      ...
      <article className="prose prose-lg dark:prose-invert max-w-4xl mx-auto px-4 py-12">
        ...
        <header className="mb-8 not-prose">
          <div className="flex items-center gap-2 mb-4 text-sm">
            <span className="px-3 py-1 bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 rounded-full font-medium">
              Software
            </span>
